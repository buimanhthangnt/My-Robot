...
class DeepQ:
    ...
    def createModel(self):
        model = Sequential()
        model.add(Convolution2D(16, 3, 3, subsample=(2,2), input_shape=(img_rows,img_cols, img_channels)))
    ...
    def performAction(self, camera, pub, action, oldObser):
        twist = Twist()
        speed = 0.32
        turn = 1.0 * (action - 1) / 3
        ...
        pub.publish(twist)
        time.sleep(0.5)
        newObservation = camera.take_picture()
        allsum = np.sum(newObservation)
        diff = np.sum(newObservation - observation)
        done = diff < 20000 or diff < 12000
        reward = 10 if not done else -100
...
class TakePhoto:
    ...
    def take_picture(self):
        if self.image_received:
            # Save an image
            self.image = cv2.resize(self.image, (160, 64))
...
if __name__ == '__main__':
    ...
    img_rows, img_cols, img_channels = 64, 160, 3
    epochs = 100000
    steps = 1000
    ...
    for epoch in xrange(current_epoch+1, epochs+1, 1):
        if epoch != 1: 
            twist = Twist()
            speed = 0.2
            turn = 1.0 * (action - 1) / 10
            twist.linear.x = speed; twist.linear.y = 0; twist.linear.z = 0
            twist.angular.x = 0; twist.angular.y = 0; twist.angular.z = turn
            pub.publish(twist)
            reset_simulation()

        observation = camera.take_picture()
        cumulated_reward = 0

        # number of timesteps
        for t in xrange(steps):
            qValues = deepQ.getQValues(np.expand_dims(observation, axis=0))

            action = deepQ.selectAction(qValues, explorationRate)
            newObservation, reward, done = deepQ.performAction(camera, pub, action, observation)
